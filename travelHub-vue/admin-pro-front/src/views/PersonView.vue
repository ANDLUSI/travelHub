...
const adminView = ref({})
const originalAdminView = reactive({})  // 保存原始数据，用于取消编辑时恢复
...
onMounted(() => {
    adminView.value = store.admin
    findById(adminView.value.id).then(res => {
        ...
        adminForm.value.validate((valid) => {
            if (valid) {
                saveAdmin(adminView.value).then(res => {
                    store.saveAdmin1(adminView.value)
                    ElMessage({
                        message: '信息已更新',
                        type: 'success'
                    });

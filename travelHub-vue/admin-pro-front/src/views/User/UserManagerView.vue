...
import { page, editStatus, deleteById, pageStatus } from '@/api/user'
...
//分页查询
const queryPage = () => {
    if (queryPageParams.status == '' || queryPageParams.status ==null){
        ...
        pageStatus(queryPageParams).then((res) => {
            console.log(`output-->res`, res)
            tableData.value = res.data.records
            total.value = res.data.total
        })
...
const filterTag = (roleParam) => {
    queryPageParams.status = roleParam == -1 ? '' : roleParam
    queryPageParams.pageNo = 1 // 筛选后重置到第一页
    queryPage()
};
...
//编辑
const editStatusMethod = (id, type) => {
    editStatus(id, type).then((res) => {
        ElMessage({
            message: res.message,
            type: 'success'
        });
...
const tableData = ref([
    {
        ...
        // createTime: '2024-08-15 12:00:00'
    }
])
...
<template>
    ...
    <div style="margin-bottom: 10px; margin-top: 20px;">
        <el-select v-model="Role" placeholder="筛选用户状态" @change="filterTag" clearable>
    ...
    <el-table :data="tableData" style="width: 100%" stripe highlight-current-row>
        <el-table-column fixed prop="username" label="用户名称" width="150" header-align="center" align="center" />
        ...
        <el-table-column prop="title" label="头衔" width="180" header-align="center" align="center" />
        <el-table-column prop="status" label="用户状态" width="150" header-align="center" align="center">

            <template #default="scope">
                <span v-if="scope.row.status == 1">正常</span>
                <span v-else-if="scope.row.status == 0">封禁中</span>
            </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="用户状态修改" min-width="150" header-align="center" align="center">
            <template #default="scope">
                <el-popconfirm confirm-button-text="封禁" cancel-button-text="正常" icon-color="#FFDEAD" title="修改用户状态为"
                    @confirm="editStatusMethod(scope.row.id, 0)" @cancel="editStatusMethod(scope.row.id, 1)"
                    :popper-class="'custom-popconfirm'" width="250">
                    <template #reference>
